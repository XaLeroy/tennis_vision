...
# from track_players import predict_players
...
@app.get("/")
def root():
    return {"greeting": "Welcome to tennis vision API!"}


@app.post("/first_step")
async def first_step(
    video_file: UploadFile = File(...), bounce: int = 0, minimap: int = 0
):
    ...
    # set constants
    n_classes = 256
    save_weights_path = "tracknet/model.1"

    yolo_classes = "yolo_v3/yolov3.txt"
    yolo_weights = "yolo_v3/yolov3.weights"
    yolo_config = "yolo_v3/yolov3.cfg"

    output_video_path = f"{video_name.split('.')[0]}_out.mp4"
    ...
    if minimap == 1:
        ...
        print("game ", fps1)
        output_video = cv2.VideoWriter(
            f"{video_name.split('.')[0]}_with_map.mp4",
            fourcc,
            fps,
        ...
        minimap_video = cv2.VideoCapture("minimap.mp4")
    ...
    if bounce == 1:
        ...
        if minimap == 1:
            video = cv2.VideoCapture(f"{video_name.split('.')[0]}_with_map.mp4")
        else:
