...
def process_video():
    ...
    frame_corner = create_avg_frame(
        video_capture_object=vid_in, start_frame=0, end_frame=80, n_th_frame=10    ## 400 et 20
    )
    ...
    for frame_count in tqdm(range(vid_in_total_frames)):
        ...
        if frame_count == 0:
            court_corners = get_court_corners(image=frame_corner, mode="automatic")
            ...
            player_height_px = real_to_pixels(
                ...
                reference_len_px=int(
                    np.linalg.norm(np.array(court_corners[2]) - np.array(court_corners[3]))
                ),
        ...
        (
            ...
            image=frame_in, model_yolo_object=model_yolo, yolo_conf=YOLO_CONFIDENCE, apply_nms=True
        )
        ...
        idx_to_keep = extract_players_close_to_baseline(image=frame_in, bboxes=player_centroids)
        player_ids, player_boxes, player_centergrounds = track_player_deep_sort(
            image=frame_in,
        ...
        frame_in = cv2.resize(
            src=frame_in, dsize=(model_resolution_wh[0], model_resolution_wh[1])
        ).astype(np.float32)
        ...
        frame_out = pp_tracking.draw_player_paths(
            image=frame_out, color=COLOR_PATHS, radius=RADIUS_PATHS, thickness=THICKNESS_PATHS
        )
    ...
    store_data(list_of_arrays=ball_list).to_csv(str(PATH_OUT_STATS / "ball.csv"))
