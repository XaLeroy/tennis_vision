...
class DetectionModel:
    ...
    def detect_player_1(self, image, court_detector):
        ...
        if len(self.player_1_boxes) == 0:
            if court_detector is None:
                ...
                white_mask = cv2.warpPerspective(
                    white_ref, court_detector.court_warp_matrix[-1], image.shape[1::-1]
                )
            ...
            box_corners = (
                max(xt - margin, 0),
                max(yt - margin, 0),
                min(xb + margin, self.v_width),
                min(yb + margin, self.v_height),
            )
            trimmed_image = image[
                max(yt - margin, 0) : min(yb + margin, self.v_height),
        ...
        cv2.rectangle(
            boxes,
            (int(self.player_1_boxes[-1][0]), int(self.player_1_boxes[-1][1])),
            (int(self.player_1_boxes[-1][2]), int(self.player_1_boxes[-1][3])),
            [0, 0, 255],
            2,
        )
    ...
    def find_player_2_box(self):
        ...
        persons_sections = {
            key: [
                val,
                val
                + len(self.persons_boxes[key])
                - np.argmax([box[0] != None for box in self.persons_boxes[key][::-1]])
                - 1,
            ]
            for key, val in self.persons_first_appearance.items()
        }
        detections = []
        ...
        for det in detections:
            ...
            boxes.extend(
                self.persons_boxes[det][
                    : persons_sections[det][1] - persons_sections[det][0] + 1
                ]
            )
    ...
    def calculate_feet_positions(self, court_detector):
        ...
        for i, box in enumerate(self.player_1_boxes):
            feet_pos = np.array(
                [(box[0] + (box[2] - box[0]) / 2).item(), box[3].item()]
            ).reshape((1, 1, 2))
        ...
        for i, box in enumerate(self.player_2_boxes):
            if box[0] is not None:
                feet_pos = np.array([(box[0] + (box[2] - box[0]) / 2), box[3]]).reshape(
                    (1, 1, 2)
                )
                feet_court_pos = cv2.perspectiveTransform(
                    feet_pos, inv_mats[i]
                ).reshape(-1)
...
def create_top_view(court_detector, detection_model, xy, fps):
    ...
    for feet_pos_1, feet_pos_2 in zip(smoothed_1, smoothed_2):
        frame = court.copy()
        frame = cv2.circle(
            frame, (int(feet_pos_1[0]), int(feet_pos_1[1])), 45, (255, 0, 0), -1
        )
        if feet_pos_2[0] is not None:
            frame = cv2.circle(
                frame, (int(feet_pos_2[0]), int(feet_pos_2[1])), 45, (255, 0, 0), -1
            )
        draw_ball_position(frame, court_detector, coords[i], i)
        i += 1
        out.write(frame)
...
def interpolation(coords):
    coords = coords.copy()
    x, y = (
        [x[0] if x is not None else np.nan for x in coords],
        [x[1] if x is not None else np.nan for x in coords],
    )
...
def diff_xy(coords):
    ...
    xx, yy = (
        np.array([x[0] if x is not None else np.nan for x in diff_list]),
        np.array([x[1] if x is not None else np.nan for x in diff_list]),
    )
